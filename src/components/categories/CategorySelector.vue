...
const hasCompletePath = computed(() => {
  return (selectedLevels.value[0] !== null) && 
         ((leafOptions.value.length === 0) || (finalSelection.value !== ''));
});

// When a level changes, reset all subsequent levels
watch(selectedLevels, (newVal, oldVal) => {
  for (let i = 0; i < newVal.length; i++) {
    if (newVal[i] !== oldVal[i]) {
      // Reset subsequent levels
      selectedLevels.value = newVal.slice(0, i + 1).concat(
        Array(selectedLevels.value.length - i - 1).fill(null)
      );
  ...
  <div class="category-selector">
    ...
    >
      <label class="block text-sm font-medium text-gray-700 mb-1">
        {{ index === 0 ? 'Subcategory' : index === 1 ? 'Sub-subcategory' : `Level ${index + 2}` }}
      </label>
      <select
      ...
      >
        <option :value="null" disabled>
          {{ index === 0 ? 'Select Subcategory' : index === 1 ? 'Select Sub-subcategory' : `Select Level ${index + 2}` }}
        </option>
        <option
          v-for="(_, key) in levelData"
          :key="key"
          :value="key"
        >
          {{ key }}
    ...
    <div class="mb-3" v-if="leafOptions.length > 0">
      <label class="block text-sm font-medium text-gray-700 mb-1">Specific Type</label>
      <select
    ...
    <div class="mt-4 text-sm text-gray-600" v-if="hasCompletePath">
      <p>Selected category: <span class="font-medium">{{ props.modelValue }}</span></p>
    </div>
  </div>
</template>